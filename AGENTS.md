...
6. **Self-update check.** Read `$GYEOL_HOME/.last_update_check`. If the file does not exist or its recorded date is more than 7 days ago:
   1. Determine the update source: the contents of `$GYEOL_HOME/.repo-url` when that file exists (an install tracking a fork until upstream carries a change it depends on), otherwise `https://raw.githubusercontent.com/inureyes/gyeol/main`. Every fetch in this step is relative to that base. Fetch `<source>/VERSION` and compare with `$GYEOL_HOME/VERSION`. The version is a date in `YY.M.DD` format (no leading zeros, e.g. `26.4.11` for 2026-04-11). Compare by splitting on `.` and comparing each numeric component (year, month, day) in order; a later date means a newer version.
   2. If the upstream version is newer:
      ...
      - **Skip every harness named in `$GYEOL_HOME/.disabled-harnesses`** (one id per line — `claude`, `codex`, `gemini`, `pi` — with `#` comments and surrounding whitespace ignored). That file records a harness gyeol was deliberately removed from while it stayed installed elsewhere; do not write its config file, its skills directory, or its extension directory. Reinstating one is the bug the file exists to prevent: the reconciliations are gated only on whether the harness directory exists, and it survives the removal because it holds the harness's other extensions and skills.
...
7. **Manual updates (on-demand).** When the user requests (e.g., "gyeol 업데이트해줘", "check for updates"), run `~/.config/gyeol/scripts/update-gyeol.sh` to bypass the 7-day cycle and check immediately. It resolves the same source, and `GYEOL_REPO_URL=<base>` overrides it for one run.
